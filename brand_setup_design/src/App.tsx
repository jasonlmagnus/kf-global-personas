...
import BrandConfigurationWizard from './components/BrandConfigurationWizard';
import { PersonaData } from './types/persona';
import { generateMockPersona } from './utils/mockPersonaGenerator';

type AppStage = 'input' | 'processing' | 'review' | 'success' | 'brand-config';
...
function App() {
  const [currentStage, setCurrentStage] = useState<AppStage>('brand-config'); // Start with brand config for demo
  ...
  const handleStageClick = (stage: AppStage) => {
    if (stage === 'brand-config') {
      setCurrentStage('brand-config');
      return;
    }
  ...
  // Show brand configuration wizard
  if (currentStage === 'brand-config') {
    return <BrandConfigurationWizard />;
  }
        ...
        <div className="max-w-6xl mx-auto px-8 py-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-6">
              <button
                onClick={() => handleStageClick('brand-config')}
                className="text-sm text-slate-600 hover:text-coral-600 font-medium transition-colors duration-300"
              >
                Brand Settings
              </button>
